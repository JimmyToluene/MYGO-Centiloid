"""
Ablation: PETResNet with FiLM retained but head-level tracer embedding removed.

Sits between the submission (`PETResNet`) and `PETResNetNoFiLM` in the tracer-
conditioning ablation sweep:

    ┌─────────────────────────┬─────────┬──────┬──────────┐
    │ Variant                 │ TN  │ FiLM │ head cat │
    ├─────────────────────────┼─────────┼──────┼──────────┤
    │ PETResNet (submission)  │  ✓  │  ✓   │    ✓     │
    │ PETResNetNoHeadEmb (this)│ ✓  │  ✓   │    —     │
    │ PETResNetNoFiLM         │  ✓  │  —   │    —     │
    └─────────────────────────┴─────────┴──────┴──────────┘

Isolates the contribution of the head-level tracer embedding concatenation.
FiLM still consumes the same `tracer_emb` to generate per-stage (γ, β), so the
embedding is not deleted — only its shortcut into the regression head is.

Forward signature (`x, tracer_idx`) matches every other variant so
`dev/train.py` and `dev/evaluate.py` route the id through unchanged.
"""

import torch
import torch.nn as nn

from mygo_centiloid.model.petresnet_film import TracerNorm, FiLMBlock, ResBlock3D


class PETResNetNoHeadEmb(nn.Module):
    """
    3D ResNet-18 + TracerNorm + per-stage FiLM, head takes pooled features only.
    ...
    Forward pass:
        ...
        Stem: Conv7³(s=2) + MaxPool(s=2)    (B,  1,128³) → (B, 64,32³)
              ↓
        Stage 1: ResBlock × 2  (stride=1)   → (B,  64, 32³)  + FiLM
        Stage 2: ResBlock × 2  (stride=2)   → (B, 128, 16³)  + FiLM
        Stage 3: ResBlock × 2  (stride=2)   → (B, 256,  8³)  + FiLM
        Stage 4: ResBlock × 2  (stride=2)   → (B, 512,  4³)  + FiLM
              ↓
        Global Average Pool                 → (B, 512)
              ↓  (no tracer-emb concat — feat_dim stays 512, not 544)
    ...
    def summary(self, input_size=(1, 1, 128, 128, 128), depth: int = 4) -> None:
        bar = "=" * 88
        print(bar)
        print(f"{'PETResNet-NoHeadEmb (ablation) Architecture':^88}")
        print(bar)
